...
class ObjectsInRedis {
    ...
    _rename(id, oldName, newName, options, callback, meta) {
        ...
        if (!meta) {
            ...
            this.checkFile(id, oldName + '/_data.json', options, (err, options, meta) => {
                if (err || (meta && meta.notExists && !meta.virtualFile)) {
                    callback && callback(utils.ERRORS.ERROR_NOT_FOUND);
                    return;
                }
                return this._rename(id, oldName + '/_data.json', newName + '/_data.json', options, meta, callback);
            });
        } else {
            this.client.rename(oldDataID, newDataID, _err =>
                this.client.rename(oldMetaID, newMetaID, _err => callback));
        }
    }
            ...
            this.client.mget(keys, (err, objects) => {
                ...
                for (let k = 0; k < keys.length; k++) {
                    try {
                        ...
                        this.log.error(`${this.namespace} Cannot parse JSON ${keys[i]}: ${objects[i]}`);
                        continue;
                    }
