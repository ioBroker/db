 ...
 *
 *      Private license. You MAY NOT USE this file without permission.
 *      Permission can be hold in form of special license.
 *
 */
/* jshint -W097 */
/* jshint strict: false */
...
function ObjectsInRedis(settings) {
    ...
    function _applyView(func, params, options, callback) {
        ...
        if (func && func.map && scripts.filter && (m = func.map.match(/if\s\(doc\.type\s?===?\s?'(\w+)'\)\semit/))) {
            client.evalsha([scripts.filter, 3, params.startkey, params.endkey, m[1]], (err, objs) => {
                objs = objs || [];
        ...
        if (func && func.map && scripts.script && func.map.indexOf('doc.common.engineType') !== -1) {
            client.evalsha([scripts.script, 2, params.startkey, params.endkey], (err, objs) => {
                objs = objs || [];
                result.rows = objs.map(obj => {
                    obj = JSON.parse(obj);
                    return {id: obj._id, value: obj};
        ...
        if (func && func.map && scripts.programs && func.map.indexOf('doc.native.TypeName === \'PROGRAM\'') !== -1) {
            client.evalsha([scripts.programs, 2, params.startkey, params.endkey], (err, objs) => {
                objs = objs || [];
                result.rows = objs.map(obj => {
                    obj = JSON.parse(obj);
                    return {id: obj._id, value: obj};
        ...
        if (func && func.map && scripts.variables && func.map.indexOf('doc.native.TypeName === \'ALARMDP\'') !== -1) {
            client.evalsha([scripts.variables, 2, params.startkey, params.endkey], (err, objs) => {
                objs = objs || [];
                result.rows = objs.map(obj => {
                    obj = JSON.parse(obj);
                    return {id: obj._id, value: obj};
        ...
        if (func && func.map && scripts.custom && func.map.indexOf('doc.common.custom') !== -1) {
            client.evalsha([scripts.custom, 2, params.startkey, params.endkey], (err, objs) => {
                objs = objs || [];
                result.rows = objs.map(obj => {
                    obj = JSON.parse(obj);
                    return {id: obj._id, value: obj};
            ...
            client.keys(objNamespace + '*', (err, keys) => {
                ...
                keys = keys.sort().filter(key => {
                    if (key && !utils.regCheckId.test(key)) {
                        const id = key.substring(objNamespaceL);
                        if (params) {
                            if (params.startkey && id < params.startkey) return false;
                            if (params.endkey && id > params.endkey) return false;
                        }
                        return true;
                    } else {
                        return false;
