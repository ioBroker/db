...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        const fallbackToSocketIo = () => {
            ...
            this.log.silly(this.namespace + ' Initiate Fallback to SocketIo Objects (' + objectsFile + ')');
            const ObjectsSocketIo = require(objectsFile);
            const _newObjects = new ObjectsSocketIo(originalSettings);
        };

        this.client.on('error', error => {
            if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Redis ERROR Objects: (' + ignoreErrors + '/' + this.stop + ') ' + error.message + ' / ' + error.stack);
            if (this.stop) return;
            if (!ready) {
                initError = true;
        ...
        this.client.on('end', () => {
            if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event end (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('connect', () => {
            if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event connect (stop=' + this.stop + ')');
            connected = true;
        });

        this.client.on('close', () => {
            if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event close (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('reconnecting', () => {
            if (connected && !ready && !initError && !ignoreErrors) reconnectCounter++;
            if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event reconnect (reconnectCounter=' + reconnectCounter + ', stop=' + this.stop + ')');
        ...
        this.client.on('ready', () => {
            ...
            if (!ready && !this.sub) {
                ...
                this.sub.on('error', error => {
                    if (this.stop) return;
                    if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Sub Client Objects No redis connection: ' + JSON.stringify(error));
                });
                    ...
                    for (let i = 0; i < keys.length; i++) {
                        ...
                        if (objs[i]) {
                            try {
                                ...
                                console.log(this.namespace + ' Cannot execute map: ' + e.message);
                            }
                        }
                    }
                    // Calculate max
