...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        const ioRegExp = new RegExp('^' + this.objNamespace.replace(/\./g, '\\.'));
        ...
        if (this.settings.connection.port === 0) { // Port = 0 means unix socket
            ...
            this.log.debug('Redis Objects: Use File Socket for connection: ' + this.settings.connection.options.path);
            ...
            this.log.debug('Redis Objects: Use Sentinel for connection: ' + this.settings.connection.options.name + ', ' + JSON.stringify(this.settings.connection.options.sentinels));
        } else {
            ...
            this.log.debug('Redis Objects: Use Redis connection: ' + this.settings.connection.options.host + ':' + this.settings.connection.options.port);
        }
        ...
        const fallbackToSocketIo = () => {
            ...
            this.log.silly('Initiate Fallback to SocketIo Objects (' + objectsFile + ')');
            const ObjectsSocketIo = require(objectsFile);
            const _newObjects = new ObjectsSocketIo(originalSettings);
        };

        this.client.on('error', error => {
            if (this.settings.enhancedLogging) this.log.silly('Redis ERROR Objects: (' + ignoreErrors + '/' + this.stop + ') ' + error.message + ' / ' + error.stack);
            if (this.stop) return;
            if (!ready) {
                initError = true;
        ...
        this.client.on('end', () => {
            if (this.settings.enhancedLogging) this.log.silly('Objects-Redis Event end (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('connect', () => {
            if (this.settings.enhancedLogging) this.log.silly('Objects-Redis Event connect (stop=' + this.stop + ')');
            connected = true;
        });

        this.client.on('close', () => {
            if (this.settings.enhancedLogging) this.log.silly('Objects-Redis Event close (stop=' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('reconnecting', () => {
            if (connected && !ready && !initError && !ignoreErrors) reconnectCounter++;
            if (this.settings.enhancedLogging) this.log.silly('Objects-Redis Event reconnect (reconnectCounter=' + reconnectCounter + ', stop=' + this.stop + ')');
        ...
        this.client.on('ready', () => {
            ...
            if (!ready && !this.sub) {
                        ...
                        try {
                            if (ioRegExp.test(channel)) {
                                ...
                                try {
                                    ...
                                    this.log.warn(`${this.namespace} Cannot parse ${id} - ${message}: ${JSON.stringify(e)}`);
                                }
                            } else {
                                this.log.warn(`${this.namespace} Received unexpected pmessage: ${channel}`);
                            }
                        } catch (e) {
                ...
                this.sub.on('error', error => {
                    if (this.stop) return;
                    this.log.warn(this.namespace + ' Sub Client Objects No redis connection: ' + JSON.stringify(error));
                });
    ...
    _getBinaryState(id, callback) {
        this.client.getBuffer(id, (err, data) => {
            if (!err && data) {
                if (callback) callback(err, Buffer.from(data));
            } else {
    ...
    checkFile(id, name, options, flag, callback) {
        ...
        this.client.get(this.getFileId(id, name, true), (err, fileOptions) => {
    ...
    _readFile(id, name, options, callback, meta) {
        this._getBinaryState(this.getFileId(id, name, false), (err, buffer) => {
            ...
            callback(meta.notExists ? utils.ERRORS.ERROR_NOT_FOUND : err, buffer, mimeType);
        });
    }
    ...
    _rename(id, oldName, newName, options, callback, meta) {
        ...
        if (!meta) {
            ...
            this.client.get(oldDataID, (_err, data) =>
                this.client.set(newDataID, data, _err =>
                    this.client.set(newMetaID, JSON.stringify(meta), _err =>
                        this.client.del(oldDataID, _err =>
                            this.client.del(oldMetaID, callback)))));
        }
    }
    ...
    _chownFile(id, name, options, callback, meta) {
        if (meta) {
            ...
            meta.acl.owner      = options.owner;
            meta.acl.ownerGroup = options.ownerGroup;
            this.setObject(metaID, meta, null, callback);
        } else {
            // it could be dir
            const dirID = this.getFileId(id, name);
            this.client.keys(dirID + '/*', (err, keys) => {
                ...
                this.client.mget(keys, (err, metas) => {
                    ...
                    const objsFiltered = [];
                    const processed = [];
                    const start = this.fileNamespaceL + dirID.length + 1;
                    const end = '$%$meta'.length;
    ...
    chownFile(id, name, options, callback) {
        ...
        this.checkFileRights(id, null, options, utils.CONSTS.ACCESS_WRITE, (err, options, meta) => {
            if (err) {
                if (typeof callback === 'function') callback(err);
            } else {
                if (!options.acl.file.write) {
    ...
    _subscribe(pattern, options, callback) {
        if (typeof pattern === 'object' && pattern instanceof Array) {
    ...
    _objectHelper(keys, objs, callback) {
        if (!keys || !keys.length) {
            ...
            this.client.set(id, message, _err => {
                this.client.publish(id, message);
    ...
    _getObject(id, options, callback) {
        this.client.get(this.objNamespace + id, (err, obj) => {
            ...
            try {
                ...
                this.log.warn(`${this.namespace} Cannot parse ${id} - ${obj}: ${JSON.stringify(e)}`);
            }
            if (obj) {
                // Check permissions
    ...
    _getObjects(keys, options, callback, dontModify) {
        ...
        this.client.mget(_keys, (err, objs) => {
            ...
            if (err) {
                ...
                if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' redis mget ' + (!objs ? 0 : objs.length) + ' ' + _keys.length);
            }
            if (objs) {
                const dontCheck =
                    options.user === utils.CONSTS.SYSTEM_ADMIN_USER ||
    ...
    _getObjectsByPattern(pattern, options, callback) {
        this.client.keys(this.objNamespace + pattern, (err, keys) => {
            ...
            if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' redis keys ' + keys.length + ' ' + pattern);
    ...
    _setObject(id, obj, options, callback) {
        ...
        this.client.get(this.objNamespace + id, (err, oldObj) => {
            try {
                ...
                this.log.error(`${this.namespace} Cannot parse ${id} - ${oldObj}: ${JSON.stringify(e)}`);
                typeof callback === 'function' && callback(`${this.namespace} Cannot parse ${id} - ${oldObj}: ${JSON.stringify(e)}`);
                return;
            }
    ...
    _delObject(id, options, callback) {
        ...
        this.client.get(this.objNamespace + id, (err, oldObj) => {
            if (err) {
                ...
                //if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' redis get ' + id + ' ok: ' + oldObj);
            }
            if (!oldObj) {
            ...
            try {
                ...
                this.log.warn(`${this.namespace} Cannot parse ${id} - ${oldObj}: ${JSON.stringify(e)}`);
            }
            ...
            if (!utils.checkObject(oldObj, options, utils.CONSTS.ACCESS_WRITE)) {
                ...
                this.client.del(this.objNamespace + id, (err) => {
                    this.client.publish(this.objNamespace + id, 'null');
                    typeof callback === 'function' && callback(err);
                });
            }
        });
    ...
    _applyViewFunc(func, params, options, callback) {
        ...
        let m;
        // filter by type
        if (func && func.map && this.scripts.filter && (m = func.map.match(/if\s\(doc\.type\s?===?\s?'(\w+)'\)\semit/))) {
        ...
        // filter by script
        if (func && func.map && this.scripts.script && func.map.indexOf('doc.common.engineType') !== -1) {
        ...
        if (func && func.map && this.scripts.programs && func.map.indexOf('doc.native.TypeName === \'PROGRAM\'') !== -1) {
        ...
        if (func && func.map && this.scripts.variables && func.map.indexOf('doc.native.TypeName === \'ALARMDP\'') !== -1) {
        ...
        // filter by custom
        if (func && func.map && this.scripts.custom && func.map.indexOf('doc.common.custom') !== -1) {
            ...
            this.client.keys(this.objNamespace + '*', (err, keys) => {
                if (!this.client) {
                    return callback(utils.ERRORS.ERROR_DB_CLOSED);
                }
                ...
                this.client.mget(keys, (err, objs) => {
                    ...
                    for (let i = 0; i < keys.length; i++) {
                        try {
                            ...
                            this.log.error('Cannot parse JSON ' + keys[i] + ':  ' + objs[i]);
                            continue;
                        }
