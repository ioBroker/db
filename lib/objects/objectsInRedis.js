...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        const ioRegExp = new RegExp('^' + this.objNamespace.replace(/\./g, '\\.') + '[a-z]+'); // cfg.o.[a-z]+
                    ...
                    this.sub.on('pmessage', (pattern, channel, message) => {
                        setImmediate(() => {
                            ...
                            try {
                                ...
                                this.log.warn(this.namespace + ' Objects pmessage ' + channel + ' ' + message + ' ' + e.message);
                                this.log.warn(this.namespace + ' ' + e.stack);
                            }
                        });
                    });
                ...
                this.loadLuaScripts(() => {
                    ...
                    this.client.get(this.objNamespace + 'system.config', (err, obj) => {
                        if (obj) {
                            ...
                            this.log.error(`${this.namespace} Cannot read system.config: ${obj}`);
                        }
    ...
    _objectHelper(keys, objs, callback) {
        if (!keys || !keys.length) {
            ...
            this.client.set(id, message, err => {
                !err && this.client.publish(id, message);
                    ...
                    if (!options.ownerGroup) {
                        ...
                        return this.getUserGroup(options.owner, (user, groups /* , permissions */) => {
                            ...
                            this.client.set(this.objNamespace + id, message, err => {
                                if (!err) {
                                    //this.settings.enhancedLogging && this.log.silly(this.namespace + ' redis publish ' + this.objNamespace + id + ' ' + message);
        ...
        this.client.get(this.objNamespace + id, (err, oldObj) => {
            ...
            if (!utils.checkObject(oldObj, options, utils.CONSTS.ACCESS_WRITE)) {
                ...
                this.client.del(this.objNamespace + id, (err) => {
                    if (!err) {
                        //this.settings.enhancedLogging && this.log.silly(this.namespace + ' redis publish ' + this.objNamespace + id + ' null');
