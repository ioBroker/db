...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        const ioRegExp = new RegExp('^' + this.objNamespace.replace(/\./g, '\\.') + '[_A-Za-z]+'); // cfg.o.[_A-Za-z]+
        ...
        this.client.on('ready', () => {
            ...
            this.client.config('set', ['lua-time-limit', 10000], (err) => { // increase LUA timeout TODO needs better fix
                ...
                if (!this.sub) {
                    ...
                    this.sub.on('end', () => {
                        if (this.settings.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event end sub (stop=' + this.stop + ')');
                        if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
                    });
