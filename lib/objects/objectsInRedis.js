...
class ObjectsInRedis {
        ...
        this.client.on('error', error => {
            if (!ready) {
                ...
                // Second stage, seems we have a socket.io server
                    ...
                    ignoreErrors = true;
                    const ObjectsSocketIo = require('./objectsInMemClientSocketIo');
    ...
    loadLuaScripts(callback, _scripts) {
        if (!_scripts) {
            if (this.scriptFiles && this.scriptFiles.filter) {
                _scripts = [];
                for (const name in this.scriptFiles) {
                    if (!this.scriptFiles.hasOwnProperty(name)) continue;
                    const shasum = crypto.createHash('sha1');
                    const buf = Buffer.from(this.scriptFiles[name]);
                    shasum.update(buf);
                    _scripts.push({
                        name,
                        text: buf,
