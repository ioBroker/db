...
const utils       = require(path2.join(getControllerDir() || __dirname, 'objectsUtils.js'));
...
function getControllerDir() {
    ...
    return controllerPath ? path2.join(path2.dirname(controllerPath), 'lib', 'objects') : null;
}
...
class ObjectsInRedis {
        ...
        if (this.settings.connection.options) {
            if (this.settings.connection.options.retry_max_delay) {
                ...
                this.settings.connection.options.retry_strategy = (_options) => {
                    console.log('Retry?? ' + this.stop);
                    if (this.stop) return null;
                    // A function this receives an options object as parameter including the retry attempt,
                    // the total_retry_time indicating how much time passed since the last time connected,
                    // the error why the connection was lost and the number of times_connected in total.
                    // If you return a number from this function, the retry will happen exactly after this
                    // time in milliseconds. If you return a non-number, no further retry will happen and
                    // all offline commands are flushed with errors. Return an error to return this
        ...
        this.client.on('error', error => {
            if (!ready) {
                ...
                // Second stage, seems we have a socket.io server
                    ...
                    let controllerPath = getControllerDir();

                    // Apparently, checking vs null/undefined may miss the odd case of controllerPath being ""
                    // Thus we check for falsyness, which includes failing on an empty path
                    if (controllerPath) {
                        controllerPath = path2.join(path2.dirname(controllerPath), 'lib', 'objects');
                    }
                    else {
                        controllerPath = __dirname; // Fallback to local dir
                    }
                    console.log('resolved SocketIo class: ' + controllerPath);
                    const ObjectsSocketIo = require(path2.join(controllerPath, 'objectsInMemClientSocketIo.js'));
                    new ObjectsSocketIo(originalSettings);
                }
                return;
            }
            ...
            if (!ready) {
                ...
                if (typeof onChange === 'function') {
                    this.sub.on('pmessage', (pattern, channel, message) => {
                        this.log.debug(this.namespace + ' redis pmessage ', pattern, channel, message);
                        ...
                        try {
                            if (ioRegExp.test(channel)) {
                                ...
                                try {
                                    ...
                                    this.log.error(`${this.namespace} Cannot parse ${id} - ${message}: ${JSON.stringify(e)}`);
                                }
                            } else {
                                this.log.error(`${this.namespace} Received unexpected pmessage: ${channel}`);
                            }
                        } catch (e) {
                            this.log.error(this.namespace + ' pmessage ' + channel + ' ' + message + ' ' + e.message);
                            this.log.error(this.namespace + ' ' + e.stack);
                        }
                    });
                }

                this.sub.on('error', error => {
                    this.log.error(this.namespace + ' No redis connection: ' + JSON.stringify(error));
                });

                this.sub.on('ready', () => {
                    if (this.settings.connection.port === 0) {
                        this.log.info(this.namespace + ' Objects connected to redis: ' + this.settings.connection.host);
                    } else {
                        this.log.info(this.namespace + ' Objects connected to redis: ' + this.settings.connection.host + ':' + this.settings.connection.port);
                    }
    ...
    _setDefaultAcl(ids, defaultAcl) {
        if (ids && ids.length) {
            ...
            this.getObject(id, (err, obj) => {
                if (obj && !obj.acl) {
                    obj.acl = defaultAcl;
                    this.setObject(id, obj, () => setImmediate(() => this._setDefaultAcl(ids, defaultAcl)));
                } else {
    ...
    _chownFile(id, name, options, callback, meta) {
        if (meta) {
            ...
            meta.acl.owner      = options.owner;
            meta.acl.ownerGroup = options.ownerGroup;
            this.setObject(metaID, meta, callback);
        } else {
            // it could be dir
            const dirID = this.getFileId(id, name);
            this.client.keys(dirID + '/*', (err, keys) => {
    ...
    _getObject(id, options, callback) {
        this.client.get(this.objNamespace + id, (err, obj) => {
            if (err) {
                ...
                //this.log.silly(this.namespace + ' redis get ' + id + ' ok: ' + obj);
            }
            try {
                obj = obj ? JSON.parse(obj) : null;
            } catch (e) {
                this.log.error(`${this.namespace} Cannot parse ${id} - ${obj}: ${JSON.stringify(e)}`);
            }
            if (obj) {
                // Check permissions
    ...
    _delObject(id, options, callback) {
        ...
        this.client.get(this.objNamespace + id, (err, oldObj) => {
            if (err) {
                ...
                //this.log.silly(this.namespace + ' redis get ' + id + ' ok: ' + oldObj);
            }
            if (!oldObj) {
    ...
    _applyViewFunc(func, params, options, callback) {
        ...
        if (func && func.map && this.scripts.filter && (m = func.map.match(/if\s\(doc\.type\s?===?\s?'(\w+)'\)\semit/))) {
            this.client.evalsha([this.scripts.filter, 4, this.objNamespace, params.startkey, params.endkey, m[1]], (err, objs) => {
                err && this.log.error(`${this.namespace} Cannot get view ${err}`);
                objs = objs || [];
    ...
    loadLuaScripts(callback, _scripts) {
        if (!_scripts) {
            ...
            return this.client.send_command('SCRIPT', hashes, (err, arr) => {
        ...
        for (let i = 0; i < _scripts.length; i++) {
            if (!_scripts[i].loaded) {
                ...
                return this.client.send_command('SCRIPT', ['LOAD', script.text], (err, hash) => {
                    script.hash = hash;
                    script.loaded = true;
