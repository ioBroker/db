...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        this.client.on('error', error => {
            console.log('Redis ERROR Objects: (' + ignoreErrors + '/' + this.stop + ') ' + error.message + ' / ' + error.stack);
            if (this.stop) return;
            if (!ready) {
                initError = true;
        ...
        this.client.on('end', () => {
            console.log('Objects-Redis end (' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });

        this.client.on('connect', () => {
            console.log('Objects-Redis connect (' + this.stop + ')');
            connected = true;
        });

        this.client.on('close', () => {
            console.log('Objects-Redis close (' + this.stop + ')');
            if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });
        ...
        this.client.on('reconnecting', () => {
            ...
            connected = false;
            initError = false;
            console.log('Objects-Redis client will try to reconnect (' + this.stop + ')');
        });
        ...
        this.client.on('ready', () => {
            ...
            if (!ready) {
                ...
                this.sub.on('error', error => {
                    if (this.stop) return;
                    this.log.error(this.namespace + ' Sub Client Objects No redis connection: ' + JSON.stringify(error));
                });
