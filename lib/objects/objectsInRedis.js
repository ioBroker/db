...
function ObjectsInRedis(settings) {
    settings = settings || {};
    const redisNamespace  = (settings.redisNamespace || (settings.connection && settings.connection.redisNamespace)|| 'config') + '.';
    ...
    function _readFile(id, name, options, callback, meta) {
        _getBinaryState(getFileId(id, name, false), (err, buffer) => {
            ...
            callback(meta.notExists ? utils.consts.ERROR_NOT_EXISTS : err, buffer, mimeType);
        });
    }
    ...
    function _unlink(id, name, options, callback, meta) {
        if (meta && meta.notExists) {
            typeof callback === 'function' && callback(utils.consts.ERROR_NOT_EXISTS);
        } else {
            ...
            client.keys(dirID, (err, keys) => {
                ...
                client.mget(keys, (err, objs) => {
                    ...
                    const files = result.map(key => {
                        const name = key.substring(redisNamespaceL + filePrefix.length + id.length + 3, key.length - 7);
        ...
        client.get(redisNamespace + id, (err, obj) => {
            ...
            if (obj) {
                ...
                if (utils.checkObject(obj, options, utils.consts.ACCESS_READ)) {
                    ...
                    callback(utils.consts.ERROR_PERMISSION);
                }
    ...
    function _getKeys(pattern, options, callback, dontModify) {
        ...
        client.keys(redisNamespace + pattern, (err, keys) => {
            ...
            if (keys) {
                ...
                if (dontCheck) {
                    for (let i = 0; i < keys.length; i++) {
                        const id = keys[i].substring(redisNamespaceL);
                        if (r.test(id) && !fileRegEx.test(keys[i])) {
                            if (!dontModify) {
                                result.push(id);
                            } else {
                                result.push(keys[i]);
                ...
                if (options.owner) {
                    ...
                    if (!options.ownerGroup) {
                        ...
                        return that.getUserGroup(options.owner, (user, groups /* , permissions */) => {
                            ...
                            const message = JSON.stringify(obj);
                            client.set(redisNamespace + id, message, err => {
                                client.publish(redisNamespace + id, message);
                                typeof callback === 'function' && callback(err);
                            });
                        });
                    }
    ...
    function _delObject(id, options, callback) {
        ...
        client.get(redisNamespace + id, (err, oldObj) => {
            ...
            if (!oldObj) {
                return typeof callback === 'function' && callback(utils.consts.ERROR_NOT_EXISTS);
            }
            ...
            if (!utils.checkObject(oldObj, options, utils.consts.ACCESS_DELETE)) {
                typeof callback === 'function' && callback({error: utils.consts.ERROR_PERMISSION});
            } else {
        ...
        if (!id || utils.regCheckId.test(id)) {
            ...
            client.get(redisNamespace + id, (err, oldObj) => {
                ...
                if (!utils.checkObject(oldObj, options, utils.consts.ACCESS_WRITE)) {
                    return typeof callback === 'function' && callback(utils.consts.ERROR_PERMISSION);
                }
