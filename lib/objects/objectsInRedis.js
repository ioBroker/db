...
class ObjectsInRedis {
                        ...
                        try {
                            if (ioRegExp.test(channel)) {
                                ...
                                try {
                                    ...
                                    this.log.warn(`${this.namespace} Cannot process pmessage ${id} - ${message}: ${e.message}`);
                ...
                .filter(key => {
                    if (key.endsWith('$%$meta')) {
                        ...
                        if (parts.length === deepLevel) {
                            return !key.includes('/_data.json$%$'); // sort out "virtual" files that are used to mark directories
                        } else {
    ...
    _applyViewFunc(func, params, options, callback) {
        ...
        let m;
        // filter by type
        if (func && func.map && this.scripts.filter && (m = func.map.match(/if\s\(doc\.type\s?===?\s?'(\w+)'\)\semit\(([^,]+),\s?doc\s?\)/))) {
            this.client.evalsha([this.scripts.filter, 4, this.objNamespace, params.startkey, params.endkey, m[1]], (err, objs) => {
                ...
                result.rows = objs.map(obj => {
                    ...
                    } else if (m[2] && m[2].trim() === 'doc.common.name' && obj.common && obj.common.name) {
                        return {id: obj.common.name, value: obj};
                    }
                    else {
                        this.log.error(`${this.namespace} Cannot filter "' + m[2] + '": ${obj}`);
                        return {id: 'parseError', value: null};
                    }
                });
                // Calculate max
