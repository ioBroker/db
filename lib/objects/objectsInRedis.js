 ...
 *
 * Copyright (c) 2018-2019 ioBroker GmbH - All rights reserved.
 *
...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        const ioRegExp = new RegExp('^' + this.objNamespace.replace(/\./g, '\\.') + '[_A-Za-z0-9ÄÖÜäöü]+'); // cfg.o.[_A-Za-z0-9]+
            ...
            this.client.config('set', ['lua-time-limit', 10000], (err) => { // increase LUA timeout TODO needs better fix
                ...
                if (!this.subSystem && typeof onChange === 'function') {
                    ...
                    this.subSystem.on('end', () => {
                        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis System Event end sub (stop=' + this.stop + ')');
                        if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
                    });

                    this.subSystem.on('error', error => {
                        if (this.stop) return;
                        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' PubSub System client Objects No redis connection: ' + JSON.stringify(error));
                    });
                ...
                if (!this.sub && typeof onChangeUser === 'function') {
                    ...
                    this.sub.on('end', () => {
                        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event end user sub (stop=' + this.stop + ')');
                        if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
                    });

                    this.sub.on('error', error => {
                        if (this.stop) return;
                        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' PubSub user client Objects No redis connection: ' + JSON.stringify(error));
                    });
    ...
    checkFile(id, name, options, flag, callback) {
        ...
        if (!this.client) {
            return typeof callback === 'function' && callback(utils.ERRORS.ERROR_DB_CLOSED, options);
        }
    ...
    _writeFile(id, name, data, options, callback, meta) {
        ...
        if (options.virtualFile) {
            ...
            this._setBinaryState(this.getFileId(id, name, false), data, err => this.client.set(metaID, JSON.stringify(meta), _err => typeof callback === 'function' && callback(err)));
        }
    }
    ...
    _getObject(id, options, callback) {
        ...
        if (!id || typeof id !== 'string') {
            return typeof callback === 'function' && callback('invalid id ' + JSON.stringify(id));
        }
    ...
    _getKeys(pattern, options, callback, dontModify) {
        ...
        if (!pattern || typeof pattern !== 'string') {
            return typeof callback === 'function' && callback('invalid pattern ' + JSON.stringify(pattern));
        }
    ...
    _setObject(id, obj, options, callback) {
        ...
        this.client.get(this.objNamespace + id, (err, oldObj) => {
            ...
            // make a merge only if real redis is used. Because in case of IN-Memory, the settings will be merged one more time on Host
            if (this.settings.connection.type === 'redis' && oldObj && oldObj.common) {
    ...
    _extendObject(id, obj, options, callback, _iteration) {
        if (!id || typeof id !== 'string' || utils.regCheckId.test(id)) {
            ...
            this.client.get(this.objNamespace + id, (err, oldObj) => {
                ...
                oldObj = oldObj || {};
                oldObj = extend(true, oldObj, JSON.parse(JSON.stringify(obj))); // copy here to prevent "sandboxed" objects from JavaScript adapter
                oldObj._id = id;
