...
class ObjectsInRedis {
    ...
    constructor(settings) {
        ...
        this.client.on('close', () => {
            if (this.settings.connection.connection.enhancedLogging) this.log.silly(this.namespace + ' Objects-Redis Event close (stop=' + this.stop + ')');
            //if (ready && typeof this.settings.disconnected === 'function') this.settings.disconnected();
        });
            ...
            this.client.config('set', ['lua-time-limit', 10000], (err) => { // increase LUA timeout TODO needs better fix
                ...
                if (!this.sub) {
                    ...
                    this.sub.on('error', error => {
                        if (this.stop) return;
                        if (this.settings.connection.enhancedLogging) this.log.silly(this.namespace + ' Sub Client Objects No redis connection: ' + JSON.stringify(error));
                    });
